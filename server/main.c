...
main(int argc, char *argv[])
{
		...
		if (strcmp(argv[i], "-level") == 0) {
			...
			if (!argv[i]) {
				...
				if (strcmp(argv[i], "1.1") == 0)
					...
					fprintf(stderr,
							"%s: unknown CP/NET level, must be 1.1 or 1.2\n",
							argv[0]);
			}
			...
			if (!argv[i]) {
				...
				_debug = strtol(argv[i], 0, 0);
				fprintf(stderr, "debug set to 0x%x\n", _debug);
			}
			...
			if (!argv[i]) {
				...
				if (set_speed(atoi(argv[i])) < 0) {
					fprintf(stderr,
							"%s: invalid serial speed, defaulting to 38400\n",
							argv[0]);
				}
			}
	...
	for (i = 0; i < 8; ++i) {
		if (lst_to_dev[i].fname) {
			...
			if (strcmp(lst_to_dev[i].fname, "-") == 0) {
				...
				if (!lst_to_dev[i].f) {
					fprintf(stderr,
							"%s: could not open %s for lst%d output\n",
	...
	for (i = 0; i < 8; ++i) {
		if (lst_to_dev[i].f &&
			(lst_to_dev[i].f != stdout) && (lst_to_dev[i].f != stderr))
			fclose(lst_to_dev[i].f);
		if (lst_to_dev[i].fname)
			free(lst_to_dev[i].fname);
	}
...
usage(char *pname)
{
	...
	printf("    0x%02x  no debug information is output\n", 0);
	printf("    0x%02x  show packet-level data transfer\n", DEBUG_PACKET);
	printf("    0x%02x  show network-level data transfer\n", DEBUG_DATA);
	printf("    0x%02x  show other miscellaneous debug information\n",
		   DEBUG_MISC);
	printf("    0x%02x  show bytes in/out\n", DEBUG_LOW);
	printf("\n");
...
int
lst_output(int num, char *buf, int len)
{
	int i;
...
read_ini(char *fname)
{
	...
	if (fname) {
		...
		if (access(buf, R_OK)) {
			sprintf(buf, "cpnet.ini");
		} else {
            strcpy(buf, "/usr/local/etc/cpnet/cpnet.ini");
        }
	}
